...
from modules.video_system import VideoManager



from modules.image_system import ImageManager
...
from modules.music_system import MusicManager
...
from modules.games import GameManager
...
from modules.study import StudyManager
...
from modules.plugin_registry import PluginContext, PluginDescriptor, register_plugin, get_plugin, get_external_plugins, scan_external_plugins, reload_external_plugins, is_plugin_enabled
...
async def _dispatch_checkin(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "checkin", checkin_mgr.handle_command, ctx.perf,
...
async def _dispatch_tools(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "tools", tools_mgr.handle_command, ctx.perf,
...
async def _dispatch_study(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "study", study_mgr.handle_command, ctx.perf,
...
async def _dispatch_music(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "music", music_mgr.handle_command, ctx.perf,
...
async def _dispatch_video(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "video", video_mgr.handle_command, ctx.perf,
...
async def _dispatch_image(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "image", image_mgr.handle_command, ctx.perf,
...
async def _dispatch_game(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "game", game_mgr.handle_command, ctx.perf,
        ...
        scene=ctx.scene, member_nick=ctx.username,
...
async def _dispatch_novel(ctx: PluginContext) -> bool:
...
async def _dispatch_group_admin(ctx: PluginContext) -> bool:
    ...
    return await ctx.bot._time_plugin(
        ...
        "group_admin", group_admin.handle_command, ctx.perf,
...
register_plugin(PluginDescriptor(key="checkin", name="签到", priority=10, dispatch=_dispatch_checkin, description="每日签到与积分"))



register_plugin(PluginDescriptor(key="tools", name="工具", priority=20, dispatch=_dispatch_tools, description="视频解析等工具"))



register_plugin(PluginDescriptor(key="study", name="学习", priority=30, dispatch=_dispatch_study, description="语文/英语/数学题目搜索"))



register_plugin(PluginDescriptor(key="music", name="音乐", priority=40, dispatch=_dispatch_music, description="点歌与音乐"))



register_plugin(PluginDescriptor(key="video", name="视频", priority=50, dispatch=_dispatch_video, description="短视频推荐"))



register_plugin(PluginDescriptor(key="image", name="图片", priority=60, dispatch=_dispatch_image, description="随机图片"))



register_plugin(PluginDescriptor(key="game", name="娱乐", priority=70, dispatch=_dispatch_game, description="五子棋/猜成语/象棋等"))



register_plugin(PluginDescriptor(key="novel", name="小说", priority=80, dispatch=_dispatch_novel, description="在线小说阅读"))



register_plugin(PluginDescriptor(key="group_admin", name="群管", priority=90, dispatch=_dispatch_group_admin, description="群管与入群欢迎"))
...
_EXACT_KEYWORDS = {
    ...
    "学习菜单", "语文", "英语", "数学", "物理", "化学", "生物", "历史", "政治", "地理",
...
_PREFIX_KEYWORDS = [
    ...
    # 学习系统：回答按钮（回答 语文 / 物理 ...；兼容旧指令 查看答案）+ 仅答案按钮（答案 语文）
...
_FEEDBACK_FORM_URL = "https://docs.qq.com/form/page/DWXNyWGJiZE9rZHFD"
...
class MyClient(botpy.Client):
    ...
    async def _handle_message_inner(self, scene: str, target_id: str, member_openid: str,
        ...
        is_waiting = tools_mgr.is_waiting(storage_id, member_openid)
        ...
        is_gaming = game_mgr.has_active_session(storage_id)
        ...
        is_studying = study_mgr.is_awaiting_answer(storage_id)
        ...
        # 小说阅读中：翻页按钮「上一页/下一页」不是触发词，需放行给 novel 处理



        is_reading = is_feature_enabled("novel", appid=self.bot_appid) and novel_mgr._is_reading(storage_id)
...
# ============ 按钮交互回调 ============
    ...
    async def _fw_game_idiom_preroute(self, ctx: PluginContext) -> bool:
        ...
        if not game_mgr.has_idiom_session(ctx.storage_id):
    ...
    async def _fw_ai_fallback(self, ctx: PluginContext) -> bool:
        ...
        if ctx.is_at_or_dm and is_feature_enabled("ai", appid=ctx.bot_appid):
            ...
            await self._handle_ai_chat(ctx.scene, ctx.target_id, ctx.content, ctx.msg_id, ctx.event_id, ctx.event_type)
    ...
    async def on_interaction_create(self, interaction: Interaction):
        ...
        # 否则 tools_mgr 内部 state key 会变成 "g:xxx|yyy"，与 handle_command
        ...
        # 中的 "xxx|yyy" 不一致，导致用户点完按钮发链接机器人无响应。
        ...
        if button_data:
            ...
            handled = await tools_mgr.handle_callback(
            ...
            handled = await game_mgr.handle_callback(
    ...
    async def _handle_ai_chat(self, scene: str, target_id: str, content: str,
                              ...
                              msg_id: str, event_id: str, event_type: str = ""):
        ...
        try:
            ...
            ok, reply, err, pname = chat_with_ai_for_bot(
    ...
    async def _send_help(self, scene: str, target_id: str, msg_id: str, event_id: str = None):
        ...
        try:
            ...
            # 游戏工具聚合按钮（原神 + 崩铁），任一游戏插件在控制台启用时显示；全禁用则不显示
            try:
                if any(is_plugin_enabled(k) for k in ("genshin_miao", "genshin", "starrail")):
    ...
    async def _send_help_category(self, scene: str, target_id: str, category: str, msg_id: str, event_id: str = None):
        ...
        try:
            ...
            _CATEGORY_CONFIG = {
                ...
                "工具菜单": {
                    ...
                    "buttons": [
                        ...
                        [("🎬 视频解析", "视频解析"), ("🌤 天气查询", "天气 ", False), ("🎮 王者信息", "王者 ", False), ("🏥 疾病信息", "疾病信息")],
                        ...
                        [("🗑️ 垃圾分类", "垃圾分类"), ("🔤 单词详解", "单词 ", False), ("👤 我的信息", "我的信息")],
                        ...
                        ([("🔗 绑群号", "绑群号 ", False)] if is_group else []) +
                ...
                "小说菜单": {
                    ...
                    "title": "📖 小说阅读系统（古典名著）\n💡 6 本古典小说 616 回（公有领域），可在线阅读（按章节分页）",
                ...
                "游戏工具菜单": {
                    "title": "🎮 游戏工具\n💡 选择游戏：原神 / 崩铁",
                    "buttons": [
                        [("✨ 原神菜单", "原神菜单", True, ["genshin_miao", "genshin"]), ("⚡ 崩铁菜单", "崩铁菜单", True, ["starrail"])],
                        [("🔙 返回主菜单", "返回主菜单")],
                    ],
                },
                ...
                "原神菜单": {
                    "title": "✨ 原神系统\n💡 所有指令需以 # 开头\n💡 提示：先 #🔗 绑定uid 再 #🔄 更新面板",
                    "buttons": [
                        [("#🔗 绑定uid", "#原神绑定uid", False, ["genshin_miao", "genshin"]), ("#🔄 更新面板", "#更新面板", True, ["genshin_miao", "genshin"]), ("#🔍 角色面板", "#原神 ", False, ["genshin_miao", "genshin"])],
                        [("#🔎 查uid", "#原神uid ", False, ["genshin_miao", "genshin"]), ("#🔀 切换api", "#切换api", True, ["genshin_miao", "genshin"]), ("#💯 圣遗物评分", "#圣遗物评分", True, ["genshin_miao", "genshin"])],
                        [("#🗡 伤害计算", "#伤害计算", True, ["genshin_miao", "genshin"]), ("#🎲 十连", "#十连", True, ["genshin_miao", "genshin"]), ("#📖 功能说明", "#原神帮助", True, ["genshin_miao", "genshin"])],
                        [("🔙 返回主菜单", "返回主菜单")],
                    ],
                },
                ...
                "崩铁菜单": {
                    "title": "⚡ 崩坏：星穹铁道\n💡 所有指令需以 * 开头\n💡 提示：先 *🔗 绑定uid 再 *🔄 更新面板",
                    "buttons": [
                        [("*🔗 绑定uid", "*星铁绑定uid", False, ["starrail"]), ("*📋 uid列表", "*星铁uid列表", True, ["starrail"]), ("*🔄 更新面板", "*更新面板", True, ["starrail"])],
                        [("*🔀 切换账户", "*切换账户", False, ["starrail"]), ("*🗑 删除账户", "*星铁删除账户", False, ["starrail"]), ("*🔍 查角色", "*星铁 ", False, ["starrail"])],
                        [("🔙 返回主菜单", "返回主菜单")],
                    ],
                },
            }
            ...
            if category == "group_admin" and is_group:
            ...
            for row_buttons in cfg["buttons"]:
                ...
                for btn_tuple in row_buttons:
                    ...
                    if len(btn_tuple) >= 4:
                        label, cmd, enter, required = btn_tuple[0], btn_tuple[1], btn_tuple[2], btn_tuple[3]
                    elif len(btn_tuple) == 3:
                        label, cmd, enter = btn_tuple
                        required = None
                    else:
                        label, cmd = btn_tuple
                        enter = True
                    ...
                    if len(btn_tuple) >= 4 or len(btn_tuple) == 3:
                        btns.append(self._make_btn(label, cmd, enter=enter))
                    else:
                        btns.append(self._make_btn(label, cmd))
